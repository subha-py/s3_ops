...
COHESITY_API['SESSION'].headers.update(
...
### authentication
def apiauth(vip='helios.cohesity.com', username='helios', domain='local',
            password=None, newPassword=None,
            updatepw=None, prompt=None, quiet=None, helios=False,
            useApiKey=False, tenantId=None, timeout=300,
            noretry=False, regionid=None, mfaType='Totp', mfaCode=None,
            emailMfaCode=False,
            entraId=False, directoryId=None, clientId=None,
    ...
    if pwd is not None:
        ...
        pwd = __getpassword(vip=vip, username=username, password=password,
                            domain=domain, useApiKey=useApiKey, helios=helios,
                            updatepw=updatepw, prompt=prompt)
    if pwd is None:
        reportAuthError(
            'no password provided for %s/%s at %s' % (domain, username, vip),
            quiet=quiet)
        return None
    ...
    if entraId is True and vip.lower() in HELIOSENDPOINTS:
        ...
        if directoryId is None:
            directoryId = __getpassword(vip=vip, username=username,
                                        password=password, domain=domain,
                                        useApiKey=useApiKey, helios=helios,
                                        updatepw=updatepw, prompt=prompt,
                                        directoryId=True)
        if clientId is None:
            clientId = __getpassword(vip=vip, username=username,
                                     password=password, domain=domain,
                                     useApiKey=useApiKey, helios=helios,
                                     updatepw=updatepw, prompt=prompt,
                                     clientId=True)
        if clientId is not None and directoryId is not None and scope is not None:
            token = ProcessOidcToken(username=username, password=pwd,
                                     client_id=clientId, tenant_id=directoryId,
                                     scope=scope)
            if token is not None:
                COHESITY_API['HEADER'] = {'accept': 'application/json',
                                          'content-type': 'application/json',
                                          'X-OPEN-ID-AUTHZ-TOKEN': token,
                                          'User-Agent': 'pyhesity/%s' % api_version}
            else:
        ...
        HELIOSCLUSTERS = (
            COHESITY_API['SESSION'].get(URL, headers=COHESITY_API['HEADER'],
                                        verify=False, timeout=timeout)).json()
        if HELIOSCLUSTERS is not None and 'message' in HELIOSCLUSTERS:
            print(HELIOSCLUSTERS['message'])
            if 'Authentication failed' in HELIOSCLUSTERS[
                'message'] and noretry is False and prompt is not False:
                apiauth(vip=vip, username=username, domain=domain,
                        updatepw=True, prompt=prompt, helios=helios,
                        useApiKey=useApiKey, quiet=True)
            else:
                reportAuthError('Helios/MCM authentication failed',
                                quiet=quiet)
                return None
        if HELIOSCLUSTERS is not None and 'errorCode' not in HELIOSCLUSTERS:
            CONNECTEDHELIOSCLUSTERS = [cluster for cluster in HELIOSCLUSTERS if
                                       cluster['connectedToCluster'] is True]
        ...
        URL = COHESITY_API['APIROOTMCM'] + 'clusters/connectionStatus'
        try:
            HELIOSCLUSTERS = (COHESITY_API['SESSION'].get(URL,
                                                          headers=COHESITY_API[
                                                              'HEADER'],
                                                          verify=False,
            ...
            if HELIOSCLUSTERS is not None and 'message' in HELIOSCLUSTERS:
                ...
                if 'Authentication failed' in HELIOSCLUSTERS[
                    ...
                    return None
            if HELIOSCLUSTERS is not None and 'errorCode' not in HELIOSCLUSTERS:
                CONNECTEDHELIOSCLUSTERS = [cluster for cluster in
                                           HELIOSCLUSTERS if cluster[
                                               'connectedToCluster'] is True]
                COHESITY_API['AUTHENTICATED'] = True
                COHESITY_API['LAST_ERROR'] = 'OK'
                if quiet is None:
                    print("Connected!")
                ...
                REGIONS = (COHESITY_API['SESSION'].get(URL,
                                                       headers=COHESITY_API[
                                                           'HEADER'],
                                                       verify=False,
                                                       timeout=timeout)).json()
            ...
            if setpasswd is not None:
                pwd = __getpassword(vip=vip, username=username,
                                    password=setpasswd, domain=domain,
                                    useApiKey=useApiKey, helios=helios,
                                    updatepw=updatepw, prompt=prompt)
            COHESITY_API['COOKIES'] = COHESITY_API[
        ...
        if cluster is not None and 'preferences' in cluster:
            ...
            COHESITY_API['AUTHENTICATED'] = False
            if 'API Key does not exist' in COHESITY_API[
                'LAST_ERROR'] or 'StatusUnauthorized' in COHESITY_API[
                'LAST_ERROR'] or 'invalid header value' in COHESITY_API[
                'LAST_ERROR']:
        ...
        creds = json.dumps(
            {"domain": domain, "password": pwd, "username": username})
        url = 'https://' + vip + '/login'
        try:
            response = COHESITY_API['SESSION'].post(url, data=creds,
                                                    headers=COHESITY_API[
                                                        'HEADER'],
                                                    verify=False,
                                                    timeout=timeout)
            if response != '':
                if response.status_code == 201 or response.status_code == 200:
                    ...
                    try:
                        ...
                        user = response.json()
                        if 'user' in user and user[
                            'user'] is not None and 'forcePasswordChange' in \
                            ...
                            if prompt is not False:
                                ...
                                reportAuthError('password is expired',
                                                quiet=quiet)
                                return None
                        else:
                            if newPassword is not None:
                        ...
                        if changePassword is True:
                            ...
                            creds = json.dumps(
                                {"domain": domain, "password": newPassword,
                                 "username": username})
                            response = COHESITY_API['SESSION'].post(url,
                                                                    data=creds,
                                                                    headers=
                                                                    COHESITY_API[
                                                                        'HEADER'],
                    ...
                    if mfaCode is not None or emailMfaCode is True:
                        ...
                        if emailMfaCode is True:
                            if user['user']['mfaInfo'][
                                'isEmailOtpSetupDone'] is False:
                                reportAuthError(
                                    'Email MFA is not enabled for user',
                                    quiet=quiet)
                                return None
                        ...
                        try:
                            ...
                            if 'errorCode' in response.json():
                                if response.json()[
                                    ...
                                    reportAuthError('MFA verification failed',
                                                    quiet=quiet)
                                    return None
                        except Exception as e:
                            reportAuthError(e, quiet=quiet)
                    ...
                    COHESITY_API['AUTHENTICATED'] = False
                    if response.status_code == 403 and 'user does not have the privilege to access ui' in \
                            response.json()['message'].lower():
                        # =============================================================================================================
                        creds = json.dumps({"domain": domain, "password": pwd,
                                            "username": username,
                                            "otpType": mfaType,
                                            "otpCode": mfaCode})
                        emailcreds = json.dumps(
                            {"domain": domain, "password": pwd,
                             "username": username})
                        ...
                        try:
                            if emailMfaCode is True:
                                print(
                                    'scripted MFA via email is disabled, please use -m xxxxxx')
                                apidrop()
                                return None
                            ...
                            if response != '':
                                if response.status_code == 201:
                                    ...
                                    # try session auth
                                    if response.status_code == 400 and 'access denied' in \
                                            response.json()['message'].lower():
                                        try:
                                            url = COHESITY_API[
                                                      'APIROOTv2'] + 'users/sessions'
                                            creds = json.dumps(
                                                {"domain": domain,
                                                 "password": pwd,
                                                 "username": username,
                                                 "otpType": mfaType.lower(),
                                                 "otpCode": mfaCode})
                                            if emailMfaCode is True:
                                                creds = json.dumps(
                                                    {"domain": domain,
                                            ...
                                            if response != '':
                                                if response.status_code == 201:
                                                    ...
                                                    COHESITY_API[
                                                        'AUTHENTICATED'] = False
                                                    COHESITY_API[
                                                        'LAST_ERROR'] = 'Error %s' % response.status_code
                                                    __writelog(
                                                        'Error %s' % response.status_code)
                                                    if quiet is None:
                                                        print(
                                                            'Error %s' % response.status_code)
                                                    if response.status_code == 400 and 'invalid username' in \
                                                            response.json()[
                                                                'message'].lower():
                    ...
                    if quiet is None:
                        print(COHESITY_API['LAST_ERROR'])
                    if (
                            response.status_code == 400 or response.status_code == 401) and 'invalid username' in \
                            response.json()['message'].lower():
                        if noretry is False and prompt is not False and setpasswd is None:
                            apiauth(vip=vip, username=username, domain=domain,
                                    updatepw=True, prompt=prompt,
                                    helios=helios, useApiKey=useApiKey)
        except requests.exceptions.RequestException as e:
            reportAuthError(e, quiet=quiet)


def ProcessOidcToken(username, password, client_id, tenant_id,
                     scope='openid profile'):
    callazure = None
    Azbody = {
        "grant_type": "password",
    ...
    # try:
    callazure = COHESITY_API['SESSION'].post(AzureURL, data=Azbody,
                                             headers=AzureHeader, verify=False)
...
def impersonate(tenantId):
    if COHESITY_API['AUTHENTICATED'] is True:
        ...
        if tenants is not None and len(tenants) > 0:
            thistenant = [t for t in tenants if
                          t['name'].lower() == tenantId.lower()]
            if thistenant is not None and len(thistenant) > 0:
                COHESITY_API['HEADER']['x-impersonate-tenant-id'] = \
                thistenant[0]['tenantId']
            else:
...
def heliosCluster(clusterName=None, verbose=False):
    ...
    if clusterName is not None:
        ...
        accessCluster = [cluster for cluster in CONNECTEDHELIOSCLUSTERS if
                         cluster['name'].lower() == clusterName.lower()]
        if not accessCluster:
            print('Cluster %s not connected to Helios' % clusterName)
            COHESITY_API[
                'LAST_ERROR'] = 'Cluster %s not connected to Helios' % clusterName
        else:
            COHESITY_API['HEADER']['accessClusterId'] = str(
                accessCluster[0]['clusterId'])
            COHESITY_API['LAST_ERROR'] = 'OK'
            if verbose is True:
                print('Using %s' % clusterName)
    else:
        print("\n{0:<20}{1:<36}{2}".format('ClusterID', 'SoftwareVersion',
                                           "ClusterName"))
        print("{0:<20}{1:<36}{2}".format('---------', '---------------',
                                         "-----------"))
        for cluster in sorted(CONNECTEDHELIOSCLUSTERS,
                              key=lambda cluster: cluster['name'].lower()):
            print("{0:<20}{1:<36}{2}".format(cluster['clusterId'],
                                             cluster['softwareVersion'],
                                             cluster['name']))


def heliosClusters():
    return sorted(CONNECTEDHELIOSCLUSTERS,
                  key=lambda cluster: cluster['name'].lower())


### api call function
def api(method, uri, data=None, quiet=None, mcm=None, mcmv2=None, v=1,
        reportingv2=None, context=None, timeout=300):
    """api call function"""
    ...
    if method in APIMETHODS:
        try:
            if method == 'get':
                response = COHESITY_API['SESSION'].get(url,
                                                       headers=THISCONTEXT[
                                                           'HEADER'],
                                                       verify=False,
                                                       timeout=timeout)
            if method == 'post':
                response = COHESITY_API['SESSION'].post(url,
                                                        ...
                                                        timeout=timeout)
            if method == 'put':
                response = COHESITY_API['SESSION'].put(url,
                                                       headers=THISCONTEXT[
                                                           'HEADER'],
                                                       json=data, verify=False,
                                                       timeout=timeout)
            if method == 'delete':
                response = COHESITY_API['SESSION'].delete(url,
                                                          headers=THISCONTEXT[
                                                              'HEADER'],
                                                          json=data,
                                                          verify=False,
            ...
            if responsejson is not None:
                if 'errorCode' in responsejson:
                    if 'message' in responsejson:
                        COHESITY_API['LAST_ERROR'] = responsejson['errorCode'][
                                                     1:] + ': ' + responsejson[
                                                         'message']
                        if quiet is None:
                            print(responsejson['errorCode'][1:] + ': ' +
                                  responsejson['message'])
                            return {
                                'error': responsejson['errorCode'][1:] + ': ' +
                                         responsejson['message']}
                        else:
                            return None
                    else:
...
def __getpassword(vip, username, password, domain, useApiKey=False,
                  helios=False, updatepw=False, prompt=True, directoryId=False,
                  clientId=False):
    ...
    if os.path.exists(PWFILE):
        ...
        for pwditem in pwdlist:
            try:
                ...
                if v.lower() == vip.lower() and d.lower() == domain.lower() and u.lower() == username.lower() and k == str(
                        useApiKey):
                    if password is not None:
                        setpwd(v=vip, u=username, d=domain, helios=helios,
                               useApiKey=useApiKey, password=password)
                        return password
                    if updatepw is not None:
                        ...
                        return base64.b64decode(opwd.encode('utf-8')).decode(
                            'utf-8')
            except Exception:
                pass
    pwpath = os.path.join(CONFIGDIR,
                          vip + '-' + domain + '-' + username + '-' + str(
                              useApiKey))
    if password is not None:
        pwd = password
        try:
    ...
    try:
        ...
        if prompt is not False:
            ...
            if directoryId is True:
                pwd = getpass.getpass("Enter Directory ID for %s at %s: " % (
                originalUsername, originalVip))
            elif clientId is True:
                pwd = getpass.getpass("Enter Client ID for %s at %s: " % (
                originalUsername, originalVip))
            elif useApiKey is True:
                pwd = getpass.getpass("Enter API Key for %s at %s: " % (
                originalUsername, originalVip))
            else:
                pwd = getpass.getpass("Enter password for %s at %s: " % (
                originalUsername, originalVip))
            try:
                pwdfile = open(pwpath, 'w')
                opwd = base64.b64encode(pwd.encode('utf-8')).decode('utf-8')
...
def setpwd(v='helios.cohesity.com', u='helios', d='local', useApiKey=False,
           helios=False, password=None, entraId=False, directoryId=False,
           clientId=False):
    originalUsername = u
    originalVip = v
...
def pw(vip, username, domain='local', password=None, updatepw=None,
       useApiKey=False, helios=False, prompt=None):
    return __getpassword(vip, username, password, domain, useApiKey, helios,
                         updatepw, prompt)


### store password from input
def storePasswordFromInput(vip, username, password, domain='local',
                           useApiKey=False, helios=False):
    if domain.lower() != 'local' and helios is False and vip.lower() not in HELIOSENDPOINTS and useApiKey is False:
        vip = '--'  # wildcard vip
    pwpath = os.path.join(CONFIGDIR,
                          vip + '-' + domain + '-' + username + '-' + str(
...
def __writelog(logmessage):
    ...
    if apierror == lastapierror and apierrorusecs < (
            lastapierrorusecs + 5000000):
        time.sleep(5)
    try:
        # output log message
...
def fileDownload(uri, fileName, v=1, timeout=300):
    ...
    response = COHESITY_API['SESSION'].get(url, headers=COHESITY_API['HEADER'],
...
def fileUpload(uri, fileName, v=1, timeout=300):
    ...
    try:
        ...
        response = COHESITY_API['SESSION'].post(url, files=files,
                                                headers=COHESITY_API['HEADER'],
                                                verify=False, timeout=timeout,
                                                cookies=COHESITY_API[
                                                    'COOKIES'])
    except Exception as e:
...
def testProp(obj, path=None):
    if obj is not None:
        if path is None or (path in obj and obj[path] is not None) and len(
                obj[path]) > 0:
            return True
    return False
...
def setContext(context):
    ...
    if isinstance(context,
                  dict) and 'HEADER' in context and 'APIROOT' in context and 'APIROOTv2' in context:
